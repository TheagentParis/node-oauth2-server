...
RefreshTokenGrantType.prototype.getRefreshToken = function(request, client) {
  ...
  return promisify(this.model.getRefreshToken, 1).call(this.model, request.body.refresh_token, client.id || null)
    .then(function(token) {
      if (!token) {
      ...
      if (token.refreshTokenExpiresAt && token.refreshTokenExpiresAt < new Date()) {
        throw new ExpiredTokenError('Invalid grant: refresh token has expired');
      }
