...
AbstractGrantType.prototype.getAccessTokenExpiresAt = function(client, user, scope) {
  const defaultAccessTokenLifetime =  new Date(Date.now() + this.accessTokenLifetime * 1000);
  if (this.model.getAccessTokenExpiresAt) {
    return promisify(this.model.getAccessTokenExpiresAt, 3).call(this.model, client, user, scope)
      .then(function(accessTokenExpire) {
        return accessTokenExpire || defaultAccessTokenLifetime;
      });
  }
